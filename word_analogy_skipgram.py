...
def main():
    ...
    ##
    if torch.cuda.is_available():
        model = torch.load('model/skip.epoch5.model')
    else :
        model = torch.load('model/skip.epoch9.model',  map_location='cpu')
    ...
    for inferenceIdx in range(len(file.questionWord)):
        ...
        result = torch.nn.functional.normalize(targetVec, dim=0) + torch.nn.functional.normalize(plusVec, dim=0) - torch.nn.functional.normalize(minusVec, dim=0)
        ...
        print(argword0[0]+' '+argword1[0]+' '+argword2[0]+' '+argword3[0]+' '+argword4[0])
